		...
		try {
			...
			for (int s = 0; s < listOfUser.getLength(); s++) {
				...
				if (systemNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					// Certificate
					NodeList userIdList = systemElement
					...
					if (userIdElement != null) {
						if (userIdElement.getChildNodes().item(0) != null) {
							strResult = userIdElement.getChildNodes().item(0)
									.getNodeValue().trim()
									+ "|";
						}
					}
					...
					// Assertion
					NodeList firstNameList = systemElement
					...
					if (firstNameElement != null) {
						if (firstNameElement.getChildNodes().item(0) != null) {
							strResult += firstNameElement.getChildNodes()
					...
					// Issuer
					NodeList lastNameList = systemElement
					...
					if (lastNameElement != null) {
						if (lastNameElement.getChildNodes().item(0) != null) {
							strResult += lastNameElement.getChildNodes()
									.item(0).getNodeValue().trim()
									+ "|";
						}
					}
