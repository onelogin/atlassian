...
public class TstAuthRequest {
	...
	// private static final String oneloginSamlEndpoint =
	// "http://onelogin.local/saml/signon/39854";
	...
	public static void main(String[] argv) {
		try {
			...
			// The appSettings object contain application specific settings used
			// by the SAML library
			AppSettings appSettings = new AppSettings();

			// Set the URL of the consume.jsp (or similar) file for this
			// application. The SAML Response will be posted to this URL
			appSettings.setAssertionConsumerServiceUrl("http://" + jiraHost
					+ ":8080/login.jsp");

			// Set the issuer of the authentication request. This would usually
			// be the URL of the issuing web application
			appSettings.setIssuer("http://" + jiraHost
					+ ":8080/secure/Dashboard.jspa");

			// The accSettings object contains settings specific to the users
			// account. At this point, your application must have identified the
			// users origin
			AccountSettings accSettings = new AccountSettings();

			// The URL at the Identity Provider where the authentication request
			// should be sent
